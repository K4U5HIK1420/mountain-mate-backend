...
const SECTIONS = ["overview", "audit", "users", "userMeta", "hotels", "rides", "payments", "bookings", "trips", "reviews", "raw"];
const RAW_COLLECTIONS = ["users", "userMeta", "hotels", "rides", "bookings", "trips", "reviews", "audit"];
const META = { overview: { label: "Overview", icon: Shield }, audit: { label: "Audit", icon: FileClock }, users: { label: "Users", icon: Users }, userMeta: { label: "User Meta", icon: WalletCards }, hotels: { label: "Hotels", icon: Building2 }, rides: { label: "Rides", icon: Car }, payments: { label: "Payments", icon: WalletCards }, bookings: { label: "Bookings", icon: BookOpen }, trips: { label: "Trips", icon: ArrowUpRight }, reviews: { label: "Reviews", icon: Star }, raw: { label: "Raw Ops", icon: Database } };
const COLS = { audit: [["createdAt", "When"], ["adminEmail", "Admin"], ["action", "Action"], ["targetType", "Target"], ["summary", "Summary"]], users: [["email", "Identity"], ["displayName", "Name"], ["role", "Role"], ["lastSignInAt", "Last Seen"]], userMeta: [["email", "Email"], ["displayName", "Display"], ["userId", "User Id"], ["updatedAt", "Updated"]], hotels: [["hotelName", "Stay"], ["location", "Location"], ["status", "Status"], ["pricePerNight", "Price"]], rides: [["vehicleType", "Ride"], ["routeFrom", "From"], ["routeTo", "To"], ["status", "Status"]], payments: [["customerName", "Customer"], ["listingLabel", "Listing"], ["amount", "Amount"], ["paymentStatus", "Payment"], ["submittedAt", "Submitted"]], bookings: [["customerName", "Customer"], ["listingLabel", "Listing"], ["status", "Status"], ["paymentStatus", "Payment"]], trips: [["title", "Trip"], ["userId", "Owner"], ["status", "Status"], ["updatedAt", "Updated"]], reviews: [["customerName", "Reviewer"], ["hotelName", "Hotel"], ["rating", "Rating"], ["createdAt", "Created"]] };
...
function StatusPill({ value }) {
  ...
  const cls = ["approved", "paid", "confirmed", "completed"].includes(v)
    ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-300"
    : ["rejected", "cancelled", "failed", "declined"].includes(v)
...
function defaultSortKey(section) {
  if (section === "users") return "createdAt";
...
function PaymentDetailsPanel({ selected, reviewReason, setReviewReason, paymentActionLoading, onApprove, onDecline }) {
  return (
    <div className="space-y-5">
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-[20px] border border-white/10 bg-black/25 p-4 text-sm text-white/80">
          <p className="text-[10px] font-black uppercase tracking-[0.24em] text-orange-300">Payment</p>
          <p className="mt-3"><span className="text-white/45">Customer:</span> {selected.customerName || "--"}</p>
          <p className="mt-1"><span className="text-white/45">Listing:</span> {selected.listingLabel || "--"}</p>
          <p className="mt-1"><span className="text-white/45">Amount:</span> Rs {selected.amount || 0}</p>
          <p className="mt-1"><span className="text-white/45">Method:</span> {selected.paymentMethod || "upi_qr"}</p>
          <p className="mt-1"><span className="text-white/45">Status:</span> <span className="ml-1 inline-flex"><StatusPill value={selected.paymentStatus} /></span></p>
        </div>
        <div className="rounded-[20px] border border-white/10 bg-black/25 p-4 text-sm text-white/80">
          <p className="text-[10px] font-black uppercase tracking-[0.24em] text-orange-300">Proof</p>
          <p className="mt-3"><span className="text-white/45">UTR:</span> {selected.transactionId || "--"}</p>
          <p className="mt-1"><span className="text-white/45">Submitted:</span> {fmtDate(selected.submittedAt)}</p>
          <p className="mt-1"><span className="text-white/45">Reviewed:</span> {fmtDate(selected.reviewedAt)}</p>
          <p className="mt-1"><span className="text-white/45">Reviewed By:</span> {selected.reviewedBy || "--"}</p>
        </div>
      </div>
      ...
      {selected.screenshotUrl ? (
        <div className="rounded-[20px] border border-white/10 bg-black/25 p-4">
          <div className="flex items-center justify-between gap-4">
            <p className="text-[10px] font-black uppercase tracking-[0.24em] text-orange-300">Screenshot Proof</p>
            <a href={selected.screenshotUrl} target="_blank" rel="noreferrer" className="text-xs font-black uppercase tracking-[0.18em] text-orange-300 underline">
              Open full image
            </a>
          </div>
          <a href={selected.screenshotUrl} target="_blank" rel="noreferrer" className="mt-4 block overflow-hidden rounded-[18px] border border-white/10">
            <img src={selected.screenshotUrl} alt="Payment proof" className="max-h-[420px] w-full object-cover" />
          </a>
        </div>
      ) : (
        <div className="rounded-[20px] border border-dashed border-white/10 bg-black/20 p-5 text-sm text-white/45">
          No screenshot proof uploaded for this payment.
        </div>
      )}

      <div className="rounded-[20px] border border-white/10 bg-black/25 p-4">
        <p className="text-[10px] font-black uppercase tracking-[0.24em] text-orange-300">Customer Note</p>
        <p className="mt-3 text-sm leading-7 text-white/70">{selected.paymentNote || "No note added."}</p>
      </div>
      ...
      <label className="block text-[10px] font-black uppercase tracking-[0.24em] text-white/35">
        ...
        <textarea
          ...
          className="mt-3 w-full rounded-[24px] border border-white/10 bg-black/30 px-5 py-4 text-sm leading-7 text-white outline-none"
        />
      </label>

      <div className="flex flex-wrap gap-3">
        <Button onClick={onApprove} disabled={paymentActionLoading || selected.paymentStatus === "paid"} size="md">
          {paymentActionLoading ? <Loader2 size={16} className="animate-spin" /> : <CheckCircle2 size={16} />}
          Approve Payment
        </Button>
        <Button variant="danger" onClick={onDecline} disabled={paymentActionLoading || selected.paymentStatus === "failed"} size="md">
          {paymentActionLoading ? <Loader2 size={16} className="animate-spin" /> : <Ban size={16} />}
          Decline Payment
        </Button>
      </div>
    </div>
...
export default function AdminDashboard() {
  ...
  const statCards = useMemo(() => {
    ...
    if (!t) return [];
    return [{ title: "Auth Users", value: t.users, sublabel: "Supabase", accent: "from-orange-500/25 to-transparent" }, { title: "User Meta", value: t.userMeta, sublabel: "profiles", accent: "from-violet-400/20 to-transparent" }, { title: "Live Stays", value: t.hotels, sublabel: `${t.pendingHotels} pending`, accent: "from-amber-400/20 to-transparent" }, { title: "Fleet", value: t.rides, sublabel: `${t.pendingRides} pending`, accent: "from-sky-400/20 to-transparent" }, { title: "Payments", value: t.pendingPayments || 0, sublabel: "awaiting review", accent: "from-orange-400/20 to-transparent" }, { title: "Bookings", value: t.bookings, sublabel: `${t.pendingBookings} pending`, accent: "from-emerald-400/20 to-transparent" }, { title: "Audit Logs", value: t.audits, sublabel: "history", accent: "from-cyan-400/20 to-transparent" }];
  }, [overview]);
  ...
  const loadOverview = async () => { const res = await API.get("/admin-console/overview"); const nextOverview = res.data?.data || null; setOverview(nextOverview); setPaymentQueueCount(nextOverview?.totals?.pendingPayments || 0); };
  ...
  useEffect(() => {
    ...
    const handlePaymentQueueUpdate = async (payload) => {
      if (typeof payload?.pendingCount === "number") {
        setPaymentQueueCount(payload.pendingCount);
      }

      if (payload?.type === "submitted") {
        notify(`New payment proof from ${payload?.payment?.customerName || "a customer"}.`, "success");
      }
      if (payload?.type === "approved") {
        notify(`Payment approved for ${payload?.payment?.customerName || "customer"}.`, "success");
      }
      if (payload?.type === "declined") {
        notify(`Payment declined for ${payload?.payment?.customerName || "customer"}.`, "warning");
      }

      try {
        await loadOverview();
        if (section === "payments") {
          await loadData("payments", query, rawCollection, page, pageSize);
        }
      } catch {
        // Keep live updates non-blocking.
      }
    };

    socket.on("payment:queue-updated", handlePaymentQueueUpdate);
    return () => {
      socket.off("payment:queue-updated", handlePaymentQueueUpdate);
    };
  }, [authLoading, user, notify, section, query, rawCollection, page, pageSize, statusFilter, paymentFilter, roleFilter, actionFilter, targetFilter, sortBy, sortDir]);
  ...
  const handleSave = async () => {
    if (!selectedId || auditMode || section === "payments") return;
    setSaving(true);
    try {
  ...
  const resetViewControls = () => {
    ...
    setActionFilter("");
    setTargetFilter("");
    setSortBy(defaultSortKey(section));
    setSortDir("desc");
    setPage(1);
  };
  const savePreset = () => {
  ...
  const applyPreset = (name) => {
    ...
    setSortBy(preset.sortBy || defaultSortKey(section));
  ...
  const handleApprovePayment = async () => {
    ...
    try {
      const res = await API.patch(`/admin-console/payments/${selectedId}/approve`, { reason: paymentReviewReason });
      notify("Payment approved.", "success");
      setSelected(res.data?.data || selected);
      await refreshCurrent();
    } catch (err) {
      notify(err?.response?.data?.message || "Unable to approve payment.", "error");
    } finally {
  ...
  const handleDeclinePayment = async () => {
    ...
    setPaymentActionLoading(true);
    try {
      const res = await API.patch(`/admin-console/payments/${selectedId}/decline`, { reason: paymentReviewReason });
      notify("Payment declined.", "success");
      setSelected(res.data?.data || selected);
      await refreshCurrent();
    } catch (err) {
      notify(err?.response?.data?.message || "Unable to decline payment.", "error");
    } finally {
      setPaymentActionLoading(false);
    }
  };
            ...
            {section === "overview" ? (
              ...
              <div className="grid gap-8 2xl:grid-cols-[1.12fr_0.88fr]">
                ...
                <div className="2xl:sticky 2xl:top-32 2xl:self-start">
                  {!selected ? (
                    <div className="rounded-[34px] border border-dashed border-white/10 bg-white/[0.02] p-8 text-center text-white/35">
                      <Database size={28} className="mx-auto mb-4 text-orange-400/70" />
                      <p className="text-[10px] font-black uppercase tracking-[0.32em]">Select a record</p>
                      <p className="mt-4 text-sm leading-7">Pick any row to inspect raw data, edit fields, review audit history, or take an action.</p>
                    </div>
                  ) : (
                    <div className="rounded-[34px] border border-white/10 bg-white/[0.03] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.35)]">
                      <div className="mb-6 flex items-start justify-between gap-4">
                        <div>
                          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-orange-300">{rawMode ? `${rawCollection} detail` : `${META[section].label} detail`}</p>
                          <h3 className="mt-2 text-2xl font-black uppercase italic tracking-tight text-white">{selected.displayName || selected.email || selected.hotelName || selected.vehicleType || selected.customerName || selected.title || selected.action || "Record"}</h3>
                        </div>
                        <div className="rounded-full border border-white/10 bg-black/30 px-3 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-white/35">{selected.status || selected.role || selected.targetType || selected.paymentStatus || "live"}</div>
                      </div>
                      ...
                      {section === "users" ? (
                        <div className="space-y-5">
                          <div className="rounded-[26px] border border-white/8 bg-black/20 p-5">
                            ...
                            <div className="mt-4 grid grid-cols-2 gap-3 text-[10px] font-black uppercase tracking-[0.2em] text-white/45">
                              ...
                              <div>Bookings: {selected.activity?.bookings || 0}</div>
                              <div>Trips: {selected.activity?.trips || 0}</div>
                            </div>
                          </div>
                          <label className="block text-[10px] font-black uppercase tracking-[0.24em] text-white/35">
                            Display name
                            <input value={userDraft.displayName} onChange={(e) => setUserDraft((p) => ({ ...p, displayName: e.target.value }))} className="mt-3 w-full rounded-[24px] border border-white/10 bg-white/5 px-5 py-4 text-sm font-bold text-white outline-none focus:border-orange-500/50" />
                          </label>
                          <label className="block text-[10px] font-black uppercase tracking-[0.24em] text-white/35">
                            Role
                            <select value={userDraft.role} onChange={(e) => setUserDraft((p) => ({ ...p, role: e.target.value }))} className="mt-3 w-full rounded-[24px] border border-white/10 bg-white/5 px-5 py-4 text-sm font-bold uppercase text-white outline-none">
                        ...
                        <div className="space-y-5">
                          <HotelDetailsPanel selected={selected} patchSelected={patchSelected} />
                          <div className="grid gap-4 md:grid-cols-2">
                            <label className="block text-[10px] font-black uppercase tracking-[0.24em] text-white/35">
                              Hotel name
                              <input value={selected.hotelName || ""} onChange={(e) => patchSelected({ hotelName: e.target.value })} className="mt-3 w-full rounded-[24px] border border-white/10 bg-white/5 px-5 py-4 text-sm font-bold text-white outline-none" />
                            </label>
                            <label className="block text-[10px] font-black uppercase tracking-[0.24em] text-white/35">
                              Location
                              <input value={selected.location || ""} onChange={(e) => patchSelected({ location: e.target.value })} className="mt-3 w-full rounded-[24px] border border-white/10 bg-white/5 px-5 py-4 text-sm font-bold text-white outline-none" />
                            </label>
                            <label className="block text-[10px] font-black uppercase tracking-[0.24em] text-white/35">
                              Price per night
                              <input value={selected.pricePerNight || 0} onChange={(e) => patchSelected({ pricePerNight: Number(e.target.value) || 0 })} className="mt-3 w-full rounded-[24px] border border-white/10 bg-white/5 px-5 py-4 text-sm font-bold text-white outline-none" />
                            </label>
                            <label className="block text-[10px] font-black uppercase tracking-[0.24em] text-white/35">
                              Rooms
                              <input value={selected.roomsAvailable || 0} onChange={(e) => patchSelected({ roomsAvailable: Number(e.target.value) || 0 })} className="mt-3 w-full rounded-[24px] border border-white/10 bg-white/5 px-5 py-4 text-sm font-bold text-white outline-none" />
                            </label>
                          </div>
                          <div className="flex flex-wrap gap-3">
                            <button type="button" onClick={() => patchSelected({ status: "approved", isVerified: true })} className="inline-flex items-center gap-2 rounded-full border border-emerald-500/40 bg-emerald-500/15 px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-emerald-300 transition hover:-translate-y-0.5 hover:bg-emerald-500/25"><CheckCircle2 size={14} />Approve</button>
                            ...
                            <button type="button" onClick={() => patchSelected({ status: "rejected", isVerified: false })} className="inline-flex items-center gap-2 rounded-full border border-red-500/40 bg-red-500/15 px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-red-300 transition hover:-translate-y-0.5 hover:bg-red-500/25"><Ban size={14} />Reject</button>
                          </div>
                          <label className="block text-[10px] font-black uppercase tracking-[0.24em] text-white/35">
                            Description
                            <textarea value={selected.description || ""} onChange={(e) => patchSelected({ description: e.target.value })} rows={5} className="mt-3 w-full rounded-[26px] border border-white/10 bg-black/30 px-5 py-4 text-[12px] leading-6 text-white outline-none" />
                          </label>
                          <details className="rounded-[20px] border border-white/10 bg-black/20 p-4">
                            <summary className="cursor-pointer text-[10px] font-black uppercase tracking-[0.24em] text-white/40">Advanced JSON</summary>
                            <textarea value={rawDraft || pretty(selected)} onChange={(e) => setRawDraft(e.target.value)} rows={10} className="mt-3 w-full rounded-[20px] border border-white/10 bg-black/30 px-4 py-3 text-[12px] leading-6 text-white outline-none" />
                          </details>
                        </div>
                      ) : section === "userMeta" ? (
                        <div className="space-y-5">
                          <div className="grid gap-4 md:grid-cols-2">
                            <label className="block text-[10px] font-black uppercase tracking-[0.24em] text-white/35">
                              Display name
                              <input value={selected.displayName || ""} onChange={(e) => patchSelected({ displayName: e.target.value })} className="mt-3 w-full rounded-[24px] border border-white/10 bg-white/5 px-5 py-4 text-sm font-bold text-white outline-none" />
                            </label>
                            <label className="block text-[10px] font-black uppercase tracking-[0.24em] text-white/35">
                              Email
                              <input value={selected.email || ""} onChange={(e) => patchSelected({ email: e.target.value })} className="mt-3 w-full rounded-[24px] border border-white/10 bg-white/5 px-5 py-4 text-sm font-bold text-white outline-none" />
                            </label>
                          </div>
                          <div className="grid gap-4 md:grid-cols-2">
                            <div className="rounded-[22px] border border-white/10 bg-black/20 px-4 py-4 text-sm text-white/70">Wishlist items: {selected.wishlistCount ?? selected.wishlist?.length ?? 0}</div>
                            <div className="rounded-[22px] border border-white/10 bg-black/20 px-4 py-4 text-sm text-white/70">Referral invites: {selected.inviteCount ?? selected.referral?.invitedUsers?.length ?? 0}</div>
                          </div>
                          <label className="block text-[10px] font-black uppercase tracking-[0.24em] text-white/35">
                            Referral code
                            <input value={selected.referral?.code || ""} onChange={(e) => patchSelected({ referral: { ...(selected.referral || {}), code: e.target.value } })} className="mt-3 w-full rounded-[24px] border border-white/10 bg-white/5 px-5 py-4 text-sm font-bold text-white outline-none" />
                          </label>
                          <details className="rounded-[20px] border border-white/10 bg-black/20 p-4">
                            <summary className="cursor-pointer text-[10px] font-black uppercase tracking-[0.24em] text-white/40">Advanced JSON</summary>
                            <textarea value={rawDraft || pretty(selected)} onChange={(e) => setRawDraft(e.target.value)} rows={10} className="mt-3 w-full rounded-[20px] border border-white/10 bg-black/30 px-4 py-3 text-[12px] leading-6 text-white outline-none" />
                        ...
                        <PaymentDetailsPanel
                          ...
                          setReviewReason={setPaymentReviewReason}
                        ...
                        <div className="space-y-5">
                          ...
                          {section === "rides" && !rawMode && <RideDetailsPanel selected={selected} patchSelected={patchSelected} />}
                          {section === "bookings" && !rawMode && (
                            <div className="grid gap-4 md:grid-cols-2">
                              <label className="block text-[10px] font-black uppercase tracking-[0.24em] text-white/35">
                                Booking status
                                <select value={selected.status || "pending"} onChange={(e) => patchSelected({ status: e.target.value })} className="mt-3 w-full rounded-[24px] border border-white/10 bg-white/5 px-5 py-4 text-sm font-bold uppercase text-white outline-none">
                                  {["pending", "confirmed", "completed", "cancelled"].map((item) => <option key={item} value={item}>{item}</option>)}
                                </select>
                              </label>
                              <label className="block text-[10px] font-black uppercase tracking-[0.24em] text-white/35">
                                Payment status
                                <select value={selected.paymentStatus || "pending"} onChange={(e) => patchSelected({ paymentStatus: e.target.value })} className="mt-3 w-full rounded-[24px] border border-white/10 bg-white/5 px-5 py-4 text-sm font-bold uppercase text-white outline-none">
                                  {["pending", "under_review", "paid", "failed"].map((item) => <option key={item} value={item}>{item}</option>)}
                                </select>
                              </label>
                            </div>
                          )}
                          <label className="block text-[10px] font-black uppercase tracking-[0.24em] text-white/35">
                            Raw editor
                            <textarea value={rawDraft || pretty(selected)} onChange={(e) => setRawDraft(e.target.value)} rows={18} disabled={auditMode} className="mt-3 w-full rounded-[26px] border border-white/10 bg-black/30 px-5 py-4 text-[12px] leading-6 text-white outline-none focus:border-orange-500/50 disabled:opacity-70" />
                          </label>
                        </div>
                      )}

                      {!auditMode && section !== "payments" && (
                        <div className="mt-6 space-y-4">
                          <Input value={confirmText} onChange={(e) => setConfirmText(e.target.value)} placeholder="Type CONFIRM before destructive action" />
                          <div className="flex flex-wrap gap-3">
                            <Button onClick={handleSave} disabled={saving} size="md">{saving ? <Loader2 size={16} className="animate-spin" /> : <CheckCircle2 size={16} />}Save changes</Button>
                            <Button variant="danger" size="md" onClick={handleDelete} disabled={destroying || !destructiveUnlocked}>{destroying ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />}{section === "users" ? "Terminate user" : "Delete record"}</Button>
                          </div>
                        </div>
                      )}
                    </div>
