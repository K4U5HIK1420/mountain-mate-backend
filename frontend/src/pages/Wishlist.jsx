...
export default function Wishlist() {
  ...
  const remove = async (itemType, id) => {
    try {
      ...
      setItems((prev) => prev.filter((entry) => String(entry.item?._id) !== String(id)));
    } catch {
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative z-10 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 mb-16 md:mb-24">
          <div>
            <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="flex items-center gap-3 mb-4">
              ...
              <p className="text-orange-500 font-black tracking-[0.5em] text-[8px] md:text-[10px] uppercase italic">
                Saved For Later
              </p>
            </motion.div>
            <h1 className="text-5xl md:text-8xl font-black italic uppercase tracking-tighter leading-none">
              YOUR <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 via-orange-600 to-white">WISHLIST.</span>
            </h1>
          </div>
          ...
          <div className="flex items-center gap-4 bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[25px] px-6 py-4 shadow-2xl">
            ...
            <span className="text-white font-black uppercase tracking-widest text-[11px] italic">
              {items.length} saved
            </span>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <motion.div
            ...
            className="text-center py-24 md:py-40 bg-white/[0.02] border border-dashed border-white/10 rounded-[50px] px-6"
          >
            <Sparkles size={48} className="text-white/10 mx-auto mb-6" />
            <p className="text-white/20 font-black tracking-[0.35em] uppercase italic text-xl mb-4">
              No saved items yet
            </p>
            <p className="text-white/35 max-w-xl mx-auto text-sm leading-7 mb-10">
              Save hotels and rides from the explore pages to keep track of options you want to revisit.
            </p>
            <Link
              to="/explore-stays"
              className="inline-flex items-center gap-4 bg-white text-black px-10 py-5 rounded-full font-black uppercase text-[10px] tracking-widest hover:bg-orange-600 hover:text-white transition-all shadow-3xl active:scale-95 italic"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
            <AnimatePresence mode="popLayout">
              {items.map((entry) => {
                ...
                const title = item.hotelName || item.vehicleType || item.vehicleModel || entry.itemType;
                const image = item.images?.[0] || "https://images.unsplash.com/photo-1501785888041-af3ef285b470?q=80&w=1600";
                const location =
                  item.location || [item.routeFrom, item.routeTo].filter(Boolean).join(" -> ");

                return (
                  <motion.div
                    layout
                    key={`${entry.itemType}-${item._id}`}
                  ...
                  >
                    ...
                    <div className="p-8 md:p-10 bg-gradient-to-b from-transparent to-black/40">
                      ...
                      <div className="flex items-center justify-between gap-4 pt-8 border-t border-white/5">
                        <Link
                          to={entry.itemType === "Hotel" ? "/explore-stays" : "/explore-rides"}
                          className="flex-1 bg-white text-black py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest text-center italic hover:bg-orange-600 hover:text-white transition-all shadow-xl active:scale-95"
                        >
                          View details
                        </Link>
                        <button
                          onClick={() => remove(entry.itemType, item._id)}
