...
export function AuthProvider({ children }) {
  ...
  const [isSupabaseAvailable, setIsSupabaseAvailable] = useState(!!supabase);

  useEffect(() => {
    let mounted = true;

    if (!supabase) {
      // Show fallback UI when Supabase is not available
      setSession(null);
      setLoading(false);
      return () => {
        mounted = false;
  ...
  const value = useMemo(() => {
    ...
    const token = session?.access_token || localStorage.getItem(LEGACY_TOKEN_KEY);
    return {
      user: currentUser,
      token,
      loading,
  ...
  }, [session, legacyUser, loading, isSupabaseAvailable]);
